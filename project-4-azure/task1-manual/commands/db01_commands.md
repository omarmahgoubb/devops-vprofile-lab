...
SSH from Windows into the Azure VM (replace the IP):

```bash
ssh -i <your-key>.pem azureuser@<db01-public-ip>
```

On Azure `Standard_B1s` (1 GiB RAM), add swap **before** a large `dnf` run. Without it, `dnf` can be **Killed** by the OOM killer and MariaDB will not install.
...
Update all installed packages on CentOS Stream 9 to the latest versions (`db01` already did this):
